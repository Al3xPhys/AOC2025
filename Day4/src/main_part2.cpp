...
int main()
{
    ...
    while (getline(infile, line))
    {
        ...
        for (char c : line)
        {
            row.push_back(c == '@' ? 1 : 0); // 1 for roll of paper, 0 for empty space
        }
    ...
    // Process the grid to find accessible rolls
    temp_grid = grid; // initialize temp_grid as a copy of grid
    while (accessible_rolls != -1)
    {
        accessible_rolls = count_neighbours(grid, temp_grid); // get number of accessible rolls
        if (accessible_rolls != -1)
        {
            total_accessible += accessible_rolls; // accumulate total accessible rolls
            grid = temp_grid;                     // update grid to reflect accessible rolls
        }
    }
