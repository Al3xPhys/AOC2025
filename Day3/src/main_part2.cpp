...
size_t countRemainingDigits(const vector<long long>& number, int combination_length){
    ...
    auto first_real = find_if(number.begin(), number.end(), [](long long digit){ return digit != -1; }); // find first occurrence of not -1
    ...
    auto second_real = find_if(first_real+1, number.end(), [](long long digit){ return digit == -1; }); // find first occurrence of -1 after first real digit
