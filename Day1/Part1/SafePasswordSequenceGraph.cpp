/* This is an adition of SafePassword.cpp to graph how changing the initial dial position affects the number of times the dial lands on zero */
...
bool calculate_dial_position(int& current_position, int& previous_position, char direction, int amount) {
    previous_position = current_position; // store previous position before changing
    if (direction == 'L') {
...
int main() {
    ...
    while (getline(infile, line)) {
        ...
        if (calculate_dial_position(dial_position, previous_position, direction, amount)) {
            // if move lands on zero but the counter started at zero
            zero_count++;
        }
    };
    infile.close();
